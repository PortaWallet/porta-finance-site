...
**Seat:** product (marketing site only) · **US-2**  
**Repo:** `PortaWallet/porta-finance-site`  
**Baseline:** `main` @ `b1eeb47` (#33) — Subs APK Amount/Days/Pause; Coming until Sepolia; Agents Coming  
**Date:** 2026-09-13  
...
**Out of scope:** wallet merge · TG/APK **login** stories · deploy · HOLD flips · F6.4 VerifyingPaymaster
...
| No login stories | US-2 does not claim TG ↔ APK login alignment. Auth is out of scope. |
| Marketing execute count | **Zero.** |

**Parity vocabulary**

- **Match** — site copy/chrome mirrors APK truth on `main`.
- **Gap** — APK truth is known; www is missing or soft (must not ship as Delivered).
...
Source for product seating: APK Subs chrome. Marketing does not reverse-engineer secrets from the public download APK binary.
...
## Parity matrix — APK truth vs www (`main` @ #33)

| # | APK truth | www surface | Status on `main` | Notes |
|---|-----------|-------------|------------------|-------|
| 1 | Amount / Days / Pause as named screens | `/subscriptions` | **Match** | `SUBSCRIPTIONS.screens` titles Amount / Days / Pause; page renders section. |
| 2 | Amount (USDC) + Days + Payee address | `/subscriptions` how-to | **Match** | Present in `content/subscriptions.ts` how steps. |
| 3 | Caps, merchants, pause or cancel | Teaser + Guides | **Match** | |
| 4 | Recurring pay · Coming | Types + badge | **Match** | `statusLabel: 'Coming'`. |
| 5 | Trading bot / Swap bot Coming later — not LIVE | Types + roadmap + guides | **Match** | Swap bot alias + Sepolia string on `main`. |
...
| 9 | Coming until Sepolia is live | Subs intro + What’s next + Guides | **Match** | Locked Coming until Sepolia — not Delivered. |
| 10 | Review is not execute | Subs + TOS Network | **Match** | Explicit string on Subs + `/terms` Network risk. Canonical path stays `/terms`. |
| 11 | What’s next Subs card | `#roadmap` item 01 | **Match** | Badge **Coming**; body **Coming until Sepolia is live — not Delivered**. Must never flip to Delivered. |
| 12 | What’s next Agents card | `#roadmap` item 02 | **Match** | **Coming — not Delivered** (Agents lane; not Subs chrome). |
| 13 | Agents Coming badge on teaser / `/agents` / Guides | Home + guides | **Match** | Coming until Sepolia; never Delivered. |
| 14 | Mobile APK download | Apps · Mobile | **Match** | APK link exists; status **not Live**; no public Dogfood chrome. |
...
| 20 | Login TG ↔ APK alignment | — | **Out of scope** | US-2: no login stories from this seat. |
...
| `/guides` | Index card for Subs (and Agents) | Cards Exist; Coming badges |
...
|------|-------------|--------------|----------------|
| **01 Subscriptions** | **Coming** | Coming until Sepolia is live — not Delivered; Amount, Days, Pause; Trading bot Coming later | Delivered · LIVE trading bot · F6.4 |
| **02 Agents** | **Coming** | Coming until Sepolia is live — not Delivered | Delivered |

Lede (locked on `main`): *Nothing below is live. Subscriptions and Agents are Coming until Sepolia is live — not Delivered.*

---

## Chrome-clone checklist (US-2 lock — closed by #33)

- [x] `content/subscriptions.ts` documents **Screens (APK chrome)**: **Amount**, **Days**, **Pause**
- [x] Copy cites **APK** (and Mini App / web) as open surfaces
- [x] **Coming until Sepolia is live** + **Review is not execute**
- [x] **Swap bot** / **Trading bot** Coming later — not LIVE
- [x] Page renders `SUBSCRIPTIONS.screens` + Coming badges
- [x] Guides + What’s next Subs mention Amount, Days, Pause
- [x] Agents/Guides Coming badges (until Sepolia) — never Delivered
- [x] TOS remains **`/terms`**; Network risk keeps review ≠ execute honesty
- [x] No F6.4 / VerifyingPaymaster / AcceptAll / retail CTAs
- [x] `content/holds.ts` untouched
- [x] No wallet merge; no login TG/APK stories

Chrome implementer: **#33** (merged). Prior baseline: **#22**. This US-2 PR locks the SoT doc + honesty pins only.
...
- [ ] No login TG/APK stories claimed done
...
| Artifact | Role |
|----------|------|
| This file | **APK-as-truth parity list** (US-2 field/screen SoT lock) |
