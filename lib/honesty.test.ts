...
describe('soft tip honesty', () => {
  ...
  it('Subscriptions guide clones wallet chrome — Coming, not fake LIVE', () => {
    ...
    expect(subs).toContain('Coming until retail flags ship')
  ...
  it('retail + social bots stay Coming until retail flags ship; review is not execute', () => {
    const subs = readFileSync(resolve(ROOT, 'content/subscriptions.ts'), 'utf8')
    expect(subs).toContain('Social and copy-style bots')
    expect(subs).toContain('Coming until retail flags ship')
    expect(subs).toContain('Review is not execute')
    expect(subs).not.toMatch(/statusLabel:\s*'Delivered'|F6\.4|VerifyingPaymaster/)
    const roadmap = readFileSync(resolve(ROOT, 'content/roadmap.ts'), 'utf8')
    expect(roadmap).toContain('Retail swap and social bots Coming until retail flags ship')
    expect(roadmap).toContain('review is not execute')
    expect(roadmap).toContain('Social bots and copy-style flows Coming until retail flags ship')
    const agents = readFileSync(resolve(ROOT, 'content/agents.ts'), 'utf8')
    expect(agents).toContain('Retail and social bots')
    expect(agents).toContain('Coming until retail flags ship')
    expect(agents).toContain('Review is not execute')
    const copy = readFileSync(resolve(ROOT, 'content/copy.ts'), 'utf8')
    expect(copy).toContain('Review is not execute')
    expect(copy).not.toMatch(RETAIL_CTA)
    const terms = readFileSync(resolve(ROOT, 'content/legal.ts'), 'utf8')
    expect(terms).toContain('Review is not execute')
    expect(terms).not.toMatch(/F6\.4|VerifyingPaymaster/)
  })
