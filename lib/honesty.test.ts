...
describe('PRODUCT-IA-AUDIT SoT', () => {
  const audit = readFileSync(resolve(ROOT, 'qa/PRODUCT-IA-AUDIT.md'), 'utf8')

  it('locks Subs vs Agents vs Social bots vs Retail lanes', () => {
    expect(audit).toContain('## Product lanes (system design)')
    expect(audit).toMatch(/\*\*Subscriptions \(Subs\)\*\*/)
    expect(audit).toMatch(/\*\*Agents\*\*/)
    expect(audit).toMatch(/\*\*Social bots\*\*/)
    expect(audit).toMatch(/\*\*Retail\*\*/)
    expect(audit).toContain('Subs ≠ Agents')
    expect(audit).toContain('Social bots ≠ Telegram bot')
    expect(audit).toContain('Retail ≠ Review')
    expect(audit).toContain('Guide Exists ≠ Delivered')
  })

  it('maps Exists / Coming / Execute without Delivered lies or F6.4 ship claims', () => {
    expect(audit).toContain('## Screens that Exist')
    expect(audit).toContain('/subscriptions')
    expect(audit).toContain('/agents')
    expect(audit).toContain('/guides')
    expect(audit).toContain('/terms')
    expect(audit).toContain('## Coming (not Delivered)')
    expect(audit).toContain('What’s next Subs card is **Coming**, never **Delivered**')
    expect(audit).toMatch(/Subscriptions.*Coming — not Delivered/s)
    expect(audit).toMatch(/Agents.*Coming — not Delivered/s)
    expect(audit).toContain('## Execute map')
    expect(audit).toContain('Marketing site execute count: zero')
    expect(audit).toContain('Review — not execute')
    expect(audit).toContain('Trading bot')
    expect(audit).toContain('Coming later')
    expect(audit).toContain('**not LIVE**')
    expect(audit).toContain('never **Delivered**')
    expect(audit).toContain("No `statusLabel: 'Delivered'`")
    expect(audit).toContain('must not claim F6.4 / VerifyingPaymaster as shipped')
    expect(audit).toContain('F6_4: false')
    expect(audit).toContain('No merge wallet')
    expect(audit).toContain('Canonical Terms = **`/terms`**')
    expect(audit).toContain('HOLDs file untouched')
  })
})
